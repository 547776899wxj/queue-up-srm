						...
						try{
							...
							datas.audioList.forEach(item =>{
								...
								if(item.callingSeq){
									...
									let speakCliniqueName = (item.cliniqueName.replace('取号','')).replace('评估','');
									let speakText = `请${speakCliniqueName},${item.callingSeq},${item.calling},到麻醉门诊就诊`;
